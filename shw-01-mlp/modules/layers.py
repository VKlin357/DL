...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            unbiased_var = self.var * (input.shape[0] / (input.shape[0] - 1))
            ...
            self.norm_input = self.input_centered * self.std_inv

            if self.affine:
                output = self.weight * self.norm_input + self.bias
            else:
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.affine:
            dnorm_input = grad_output * self.weight
        else:
            dnorm_input = grad_output

        if self.training:
            dvar = np.sum(dnorm_input * self.input_centered * -0.5 * self.std_inv**3, axis=0)
            dmean = np.sum(dnorm_input * -self.std_inv, axis=0) + dvar * np.mean(-2.0 * self.input_centered, axis=0)
            grad_input = (dnorm_input * self.std_inv) + (dvar * 2.0 * self.input_centered / batch_size) + (dmean / batch_size)
        else:
            grad_input = dnorm_input * self.std_inv
...
class Sequential(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        for i in reversed(range(len(self.modules))):
            ...
            grad_input = module.backward(outputs[i], grad_input)
        return grad_input
